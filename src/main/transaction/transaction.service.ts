...
export class TransactionService {
  ...
  async getMerchantBalance(id: number): Promise<BpmResponse> {
    try {
      ...
      return new BpmResponse(true, { activeBalance: (((topupBalance - 500) - withdrowBalance) - data.totalFrozenAmount) + data.totalActiveAmount, frozenBalance: data.totalFrozenAmount }, null)
    } catch (error) {
